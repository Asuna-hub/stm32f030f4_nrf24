...
void NRF24l01_init(void){
	...
	stat = NRF24_ReadReg(NRF24_REG_CONFIG);
	conf = NRF24_ReadConfig();
	delay_uS(5);
	NRF24_ClearStatus();
	NRF24_WriteReg(NRF24_REG_EN_AA, 1 << NRF24_ENAA_P1); // enable pipe1
	NRF24_WriteReg(NRF24_REG_EN_RXADDR, NRF24_ERX_P0_ON);
	NRF24_WriteReg(NRF24_REG_SETUP_AW, NRF24_SETUP_AW_3bytes);
	NRF24_WriteReg(NRF24_REG_SETUP_RETR, NRF24_SETUP_RETR_ARD_1500uS | NRF24_SETUP_RETR_ARC_15);
...
void NRF24_Set_my_addr(uint8_t *addr){
    NRF24_CE_HIGH;
    NRF24_WriteRegMultiple(NRF24_REG_RX_ADDR_P1,addr,5);
    NRF24_CE_LOW;
}
